<template>
<b-modal id="selectRoleModal" v-model="isVisibile" @hide="resetValues()" size="md" hide-header-close no-stacking :title="$t('admin.assign_role')">
    <b-card no-body>
        <b-card-body class="pb-0">
            <b-input-group-form-select id="input-selected-project" v-model="selectedProject" :options="
            availableProjects.map((project) => ({
              value: project.uuid,
              text: project.name,
            }))
          " :label="$t('admin.project')" required="true" />
        </b-card-body>
        <b-card-body class="pb-0">
            <b-input-group-form-select id="input-selected-role" v-model="selectedRole" :options="
            availableRoles.map((role) => ({
              value: role.uuid,
              text: role.name,
            }))
          " :label="$t('admin.role')" required="true" />
        </b-card-body>
    </b-card>
    <template v-slot:modal-footer="{ cancel }">
        ...
        <b-button size="md" variant="primary" @click="handleSelection">{{$t('message.assign')}}</b-button>
    </template>
</b-modal>
</template>
